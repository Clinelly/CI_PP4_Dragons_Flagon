...
class TestTavernModels(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.review = Review.objects.create(title="Title",
                                           author=testuser1,
                                           content='Content',
                                           slug='12345')
        cls.gallery = Gallery.objects.create(title="Title2",
                                             author=testuser2,
                                             content='Content',
                                             slug='54321')
        cls.gallery.likes.set([testuser1.pk, testuser2.pk])
        cls.review.likes.set([testuser1.pk, testuser2.pk])
        cls.post = Gallery.objects.create(title="Title",
                                          author=testuser1,
                                          content='Content')
        cls.comment = Comment.objects.create(body='body',
                                             name='name',
                                             post=cls.post)
        cls.expected_string = (
            f"Comment {cls.comment.body} by {cls.comment.name}"
            )
